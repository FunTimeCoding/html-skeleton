...
${FIND} . -regextype posix-extended -type f -regex "${INCLUDE_FILTER}" -exec sh -c '${1} --in-place --expression "s/HypertextSkeleton/${2}/g" --expression "s/hypertext-skeleton/${3}/g" --expression "s/hypertext_skeleton/${4}/g" "${5}"' '_' "${SED}" "${NAME}" "${DASH}" "${UNDERSCORE}" '{}' \;
# shellcheck disable=SC1117
${SED} --in-place --expression "s/bin\/hs/bin\/${INITIALS}/g" README.md Dockerfile
git mv lib/hypertext_skeleton.sh "lib/${UNDERSCORE}.sh"
git mv spec/hypertext-skeleton_spec.sh "spec/${DASH}_spec.sh"
